...
namespace Nintenlord.Event_Assembler.Core.Code.Language
{
  ...
  sealed class EACodeLanguageDisassembler
  {
    ...
    public IEnumerable<string[]> DisassembleChapter(byte[] code, int offset, ILog log, bool addEndingLines)
    {
      ...
      foreach (Tuple<int, Tuple<string, List<Priority>>> tuple in ((IEnumerable<Tuple<string, List<Priority>>[]>) this.pointerList).SelectMany<Tuple<string, List<Priority>>[], Tuple<string, List<Priority>>>((Func<Tuple<string, List<Priority>>[], IEnumerable<Tuple<string, List<Priority>>>>) (x => (IEnumerable<Tuple<string, List<Priority>>>) x)).Index<Tuple<string, List<Priority>>>())
      {
        int startIndex = offset + 4 * tuple.Item1;
        int int32 = BitConverter.ToInt32(code, startIndex);
        if (this.pointerMaker.IsAValidPointer(int32))
        {
          int num = this.pointerMaker.MakeOffset(int32);
          intList.Add(num);
          if (num > 0 && !labels.ContainsKey(num))
          {
            labels.Add(num, tuple.Item2.Item1);
            foreach (Code code1 in this.FindTemplatesUntil(code, num, sortedDictionary, (IEnumerable<Priority>) tuple.Item2.Item2, (Predicate<ICodeTemplate>) (x => x.EndingCode), log))
              sortedDictionary[code1.Offset] = code1;
          }
        }
        else
        {
          log.AddError("Invalid pointer {0} in offset {1} at pointer list.", new object[2]
          {
            (object) int32.ToHexString("$"),
            (object) startIndex
          });
      ...
      return this.GetLines((IEnumerable<KeyValuePair<int, Code>>) sortedDictionary, (IDictionary<int, string>) labels, addEndingLines);
    }
